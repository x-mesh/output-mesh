...
export const KIND = {
  ...
  IMAGE: 'image',
  OTHER: 'other',
  // 목록에서만 쓰는 종류. 추출과 미리보기는 TEXT 그대로라 kindOf 는 이 값을 내지 않는다(listKindOf).
  MEMO: 'memo',
};
...
// drawio 는 압축 없는 mxfile XML 이다. 마크업으로 두면 본문이 태그를 벗고 도형 이름만 남아
// 도형 안의 글자로 찾을 수 있다. 미리보기는 종류가 아니라 확장자로 갈라 뷰어로 보낸다.
const MARKUP_EXT = new Set(['html', 'htm', 'svg', 'drawio']);
 ...
 */
export const KIND_RULES_VERSION = 4;
...
// 에이전트가 자기용으로 적는 메모(Claude Code 의 프로젝트 memory). 산출물이 아니라서 라이브러리에서는
// 숨기지만 찾을 일은 있어서 수집에서 빼지 않는다. 실측 18개가 라이브러리와 "방금 일어난 일" 맨 위에 섞였다.
// 폴더 이름 memory 만으로 가르지 않는다 — 저장소의 docs/memory/ 는 진짜 문서다.
const AGENT_MEMO_PATH = /\/\.claude\/projects\/[^/]+\/memory\//;

/**
 * 목록에 굳히는 종류. kindOf 와 갈라 둔 이유: kindOf 는 추출과 미리보기를 구동하고 메모도 문서로 읽혀야
 * 한다. 경로를 보는 건 여기뿐이다. 고치면 KIND_RULES_VERSION 을 올린다.
 */
export function listKindOf(ext, fileName = '', absPath = '') {
  const kind = kindOf(ext, fileName);
  return kind === KIND.TEXT && AGENT_MEMO_PATH.test(absPath) ? KIND.MEMO : kind;
}
...
/**
 * drawio 의 읽을거리는 태그 사이가 아니라 `value` 속성에 있다. 태그만 벗기면 도형 이름이 통째로
 * 사라지고 남는 건 style 문자열이라, 검색이 `shape=image` 같은 것으로 찬다. 라벨은 HTML 을
 * 담을 수 있어서 한 번 더 벗긴다.
 */
export function drawioLabels(raw) {
  const labels = new Set();
  for (const [, value] of raw.matchAll(/\svalue="([^"]*)"/g)) {
    const text = normalizeWhitespace(stripTags(decodeEntities(value)));
...
export async function runCapped(cmd, cap) {
...
/**
 * 마크다운 장식을 벗긴 글자. 문서 제목과 검색 발췌가 같은 규칙을 쓴다 — 두 곳에 따로 두면
 * 한쪽만 고쳐진다. 줄 단위 표시(제목·울타리·목록·표 구분선)는 공백을 평탄화하기 전에 떼야 한다.
 *
 * 두 가지는 일부러 남긴다. 백틱 안의 글자는 `cd server && cargo test` 처럼 장식이 아니라 내용이고,
 * 홑별표·밑줄 강조는 `files_changed` 나 `2 * 3` 을 망가뜨려서 건드리지 않는다.
 */
export function plainText(text) {
  return decodeEntities(
    String(text)
      // 색인된 본문은 줄바꿈이 이미 지워져 있다(extractBody 의 normalizeWhitespace). 줄 시작으로만
      // 잡으면 발췌에서 하나도 안 걸리므로, 헷갈릴 일 없는 표시는 공백 경계로 잡는다.
      .replace(/(^|\s)(?:```|~~~)[a-zA-Z0-9_+-]*/g, '$1')
      .replace(/(^|\s)#{1,6}[ \t]+/g, '$1')
      .replace(/(^|\s)\|?[ \t]*:?-{2,}:?[ \t]*(?:\|[ \t]*:?-{2,}:?[ \t]*)*\|?(?=\s|$)/g, '$1')
      .replace(/^[ \t]*>[ \t]?/gm, '')
      // 목록 머리표는 줄 시작일 때만 뗀다. 공백 경계로 잡으면 본문의 " - " 를 함께 지운다.
      .replace(/^[ \t]*[-*+][ \t]+/gm, '')
      .replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
      .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
      .replace(/(\*\*|__|~~|`)/g, '')
...
export function cleanTitle(text) {
  const title = normalizeWhitespace(plainText(text));
 ...
 */
export const EXTRACT_RULES_VERSION = 3;
...
export async function extractBody(absPath, ext, fileName = '') {
  ...
  try {
    ...
    if (kind === KIND.TEXT || kind === KIND.CODE || kind === KIND.MARKUP) {
      ...
      const markup = () => (ext === 'drawio' ? drawioLabels(raw) : stripTags(raw));
      const body = normalizeWhitespace(kind === KIND.MARKUP ? markup() : raw);
