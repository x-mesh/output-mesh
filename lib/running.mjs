...
export const writerLockFileFor = (dbPath) => dbPath + '.writer.lock';
...
export function acquireWriterLock(dbPath, {
  pid = process.pid,
  command = 'unknown',
  at = Math.floor(Date.now() / 1000),
  alive = isAlive,
} = {}) {
  const path = writerLockFileFor(dbPath);
  mkdirSync(dirname(path), { recursive: true });
  const token = String(pid) + '-' + Date.now() + '-' + Math.random().toString(36).slice(2);
  const record = { pid, command, at, token };
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (Number.isInteger(found?.pid) && alive(found.pid)) {
      const busy = new Error('Catalog is locked by pid ' + found.pid
        + (found.command ? ' (' + found.command + ')' : '') + '.');
      busy.code = 'CATALOG_LOCKED';
      busy.owner = found;
      throw busy;
    }
    if (found === null) {
      const busy = new Error('Catalog writer lock is unavailable.');
      busy.code = 'CATALOG_LOCKED';
      throw busy;
    }
  ...
  const busy = new Error('Catalog writer lock is busy.');
  busy.code = 'CATALOG_LOCKED';
  throw busy;
}

export function releaseWriterLock(lock) {
  if (!lock) return false;
  let found;
  try {
    found = JSON.parse(readFileSync(lock.path, 'utf8'));
  } catch {
    return false;
  }
  if (found?.token !== lock.token) return false;
