...
import { DEFAULT_CHANGE_LIMIT, DEFAULT_HOST, DEFAULT_PORT, DEFAULT_SEARCH_LIMIT, MAX_CHANGE_LIMIT, MAX_DRAWIO_BYTES, MAX_LISTED_MEMBERS, MAX_PREVIEW_BYTES, MAX_SEARCH_LIMIT, nfc } from './paths.mjs';
import { activity, describeArtifact, facets, overview, periodStart, recentChanges, search, searchCount, timeline } from './search.mjs';
import { sourcesStatus } from './sources.mjs';
import { mimeFor, artifactCsp } from './mime.mjs';
import { drawioCsp, drawioPage } from './drawio.mjs';
import { peers } from './peers.mjs';
import { importPath } from './importer.mjs';
import { surveyCoverage } from './coverage.mjs';
import { readSheets } from './sheet.mjs';
import { NAME, VERSION } from './version.mjs';
...
function artifactDetail(store, id) {
  ...
  const duplicates = row.content_hash
    ...
    : [];
  // 구성 파일의 종류는 서버가 매긴다. 화면이 확장자로 다시 가르면 분류 규칙이 두 곳으로 갈린다.
  const members = row.bundle_files
    ? (store.db.query('SELECT body FROM search_docs WHERE artifact_id = ?').get(id)?.body ?? '')
        .split('\n')
        .filter(Boolean)
        ...
        .map((rel) => {
          ...
          return { path: rel, ext, kind: kindOf(ext, name) };
        })
    : undefined;
  return {
    ...row,
...
/**
 * 접힌 번들 안의 파일 하나를 가리킨다. 클라이언트가 준 이름은 DB 에 저장된 구성 목록에 **그대로**
 * 있을 때만 받는다 — 그 목록은 수집이 만든 값이지 사용자 입력이 아니다. 받은 뒤에도 폴더 밖으로
 * 나가지 않는지 다시 본다. 구성 파일에는 스크립트 허용 토글이 없으므로 항상 꺼진 채로 나간다.
 */
function bundleMember(store, id, rel) {
  const row = store.db.query('SELECT abs_path, bundle_files FROM artifacts WHERE id = ?').get(id);
  if (!row || row.bundle_files === null) return null;
  const listed = (store.db.query('SELECT body FROM search_docs WHERE artifact_id = ?').get(id)?.body ?? '').split('\n');
  if (!listed.includes(rel)) return null;

  const absPath = join(row.abs_path, rel);
  if (!resolve(absPath).startsWith(resolve(row.abs_path) + '/')) return null;
  if (!existsSync(absPath) || !statSync(absPath).isFile()) return null;

  const fileName = nfc(absPath.split('/').pop());
  const dot = fileName.lastIndexOf('.');
  const ext = dot > 0 ? fileName.slice(dot + 1).toLowerCase() : '';
  return { abs_path: absPath, ext, kind: kindOf(ext, fileName), allow_scripts: 0, size_bytes: statSync(absPath).size };
}

/** `path` 가 있으면 번들 구성 파일, 없으면 아티팩트 자신. 두 미리보기 경로가 같은 규칙을 쓴다. */
function previewTarget(store, id, member) {
  if (member) return bundleMember(store, id, member);
  // 파일 경로는 오직 DB 에서만 온다. 클라이언트 문자열이 파일시스템에 닿는 경로가 없다.
  const row = store.db.query('SELECT abs_path, ext, kind, allow_scripts, size_bytes FROM artifacts WHERE id = ?').get(id);
  return row && existsSync(row.abs_path) ? row : null;
}
  ...
  return createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && path === '/api/events') {
        ...
        res.write(`data: ${JSON.stringify({ type: 'hello', counts: store.counts(), error: watcher?.recentError() ?? null })}\n\n`);
        clients.add(res);
      ...
      // tailnet 의 다른 노드. 없으면 빈 목록이라 화면이 전환기를 감춘다.
      if (req.method === 'GET' && path === '/api/peers') return json(res, 200, await peers());

      if (req.method === 'GET' && path === '/api/facets') return json(res, 200, facets(store, filtersFrom(url), url.searchParams.get('q') ?? ''));
      if (req.method === 'GET' && path === '/api/overview') return json(res, 200, overview(store));
      if (req.method === 'GET' && path === '/api/version') return json(res, 200, { name: NAME, version: VERSION });
      if (req.method === 'GET' && path === '/api/changes') {
        const limit = Math.min(integerParam(url, 'limit') ?? DEFAULT_CHANGE_LIMIT, MAX_CHANGE_LIMIT);
        const cursor = { before: integerParam(url, 'before'), after: integerParam(url, 'after') };
        return json(res, 200, recentChanges(store, url.searchParams.get('q') ?? '', filtersFrom(url), limit, cursor));
      }
      ...
      const drawioMatch = path.match(/^\/artifact\/(\d+)\/drawio$/);
      if (req.method === 'GET' && drawioMatch) {
        const row = previewTarget(store, Number(drawioMatch[1]), url.searchParams.get('path'));
        if (!row || row.ext !== 'drawio') return json(res, 404, { error: 'not found' });
        if (row.size_bytes > MAX_DRAWIO_BYTES) return json(res, 413, { error: 'too large to preview' });
        res.writeHead(200, {
          'content-type': 'text/html; charset=utf-8',
          'content-security-policy': drawioCsp(`http://${req.headers.host}`),
          'x-content-type-options': 'nosniff',
          'referrer-policy': 'no-referrer',
          'cache-control': 'no-store',
        });
        const theme = url.searchParams.get('theme') === 'dark' ? 'dark' : 'light';
        return res.end(drawioPage(readFileSync(row.abs_path, 'utf8'), { theme }));
      }

      const rawMatch = path.match(/^\/artifact\/(\d+)\/raw$/);
      if (req.method === 'GET' && rawMatch) {
        const row = previewTarget(store, Number(rawMatch[1]), url.searchParams.get('path'));
        if (!row) return json(res, 404, { error: 'not found' });
        const kind = row.kind;
...
export function startServer(options, { port = DEFAULT_PORT, host = DEFAULT_HOST } = {}) {
  ...
  // listen 오류는 비동기로 온다. 받지 않으면 포트가 막혔을 때 잡을 수 없는 예외가 되어
  // 스택만 남고 무엇을 하면 되는지는 안 보인다.
