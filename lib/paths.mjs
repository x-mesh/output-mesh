...
// 접는 이유는 "수백 개가 한 덩어리"다. 산출물 몇 개를 담은 폴더까지 접으면 그 안의 그림과 데이터가
// 미리보기도 검색도 없이 한 줄 뒤로 사라진다. 실측 14개 번들의 구성 파일 수는 1·1·1·4·5·7 과
// 13·15·16·18·21·30·87·107 로 갈렸다.
export const BUNDLE_MIN_FILES = 10;
// 접힌 번들 상세에 늘어놓는 구성 파일 수. 107개짜리까지 다 보이고, 그보다 큰 스캐폴딩은 잘린다.
export const MAX_LISTED_MEMBERS = 200;
...
// 출처에 저장하는 작업 제목의 길이. 활동 카드 한 줄에 들어간다.
...
// drawio 는 글자가 아니라 렌더러의 입력이라 상한이 다르다. 실측 306개의 중앙값 29KB · 90% 295KB ·
// 최대 2.11MB 로 텍스트 상한에 걸린다.
export const MAX_DRAWIO_BYTES = 8 * 1024 * 1024;
// 시작할 때 compact 를 권하는 선. 빈 자리가 이만큼이면 눌러서 줄일 값이 있다.
export const COMPACT_HINT_RATIO = 0.25;
// tailnet 의 다른 노드를 찾는 데 드는 시간. 꺼진 기기는 빼고 물어도 응답이 없는 기기가 있어서
// 상한을 짧게 둔다. 목록은 자주 바뀌지 않으므로 잠깐 들고 있는다.
export const PEER_PROBE_MS = 1_500;
...
// 검색 결과의 순서. artifact_fts 의 열 순서(name, path, body, meta)대로다. 이름이나 작업 제목에
// 맞은 파일이 본문에 한 번 스친 파일보다 위에 온다.
export const SEARCH_BM25_WEIGHTS = [10, 2, 1, 4];
// 검색 결과 줄의 본문 발췌. 일치한 곳 앞을 조금 남겨야 어느 문장인지 읽힌다.
export const EXCERPT_LEAD_CHARS = 24;
...
// 에이전트가 이보다 전에 쓴 변경은 '방금 일어난 일'에 남기지 않는다. 서버가 꺼져 있던 동안 밀린 로그가
// 재시작 때 몇 주 전 파일을 '방금'으로 올렸다. 조금 전 일은 보고 싶어서 한 시간이 아니라 네 시간이다.
export const FEED_BACKLOG_S = 4 * 60 * 60;
// 수집 오류를 상단에 보이는 시간. 수집은 30초마다 다시 성공하므로 "마지막 수집이 실패했나"로는
// 한 번 난 실패가 바로 가려진다.
export const INGEST_ERROR_VISIBLE_S = 60 * 60;
