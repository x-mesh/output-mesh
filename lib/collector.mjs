...
import { BUILD_ARTIFACT_DIRS, COLLECT_YIELD_MS, MAX_BUNDLE_MEMBERS, PROJECT_MARKERS, nfc } from './paths.mjs';
...
import { SESSION_LOG_PARSER_VERSION, SESSION_LOG_SOURCES, logFilesSince } from './session-logs.mjs';
...
export async function sweepSessionLogs(store, source, { onProgress } = {}) {
  ...
  for (const [index, log] of logs.entries()) {
    ...
    readBytes += log.size;
    try {
      for (const write of source.parse(log.path)) {
...
export async function collectSessionLogs(store, sources = SESSION_LOG_SOURCES, { onProgress } = {}) {
  ...
  const out = {};
  for (const source of sources) out[source.collector] = await sweepSessionLogs(store, source, { onProgress });
  return out;
}
