...
import { PERIODIC_SWEEP_MS, WATCH_DEBOUNCE_MS } from './paths.mjs';
import { collectOnce, collectSessionLogs, indexPending } from './collector.mjs';
...
export class Watcher {
  ...
  async collect({ onProgress } = {}) {
    ...
    try {
      ...
      this.lastError = null;
      this.sweepCount++;
      this.emit({ type: 'collected', counts: this.store.counts(), at: Math.floor(Date.now() / 1000) });
      return results;
    } catch (error) {
