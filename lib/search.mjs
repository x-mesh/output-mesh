 ...
 * (이 목록 자체가 최근이다).
 */
export function recentChanges(store, input, filters = {}, limit = DEFAULT_CHANGE_LIMIT) {
  ...
  const changes = store.db
    .query(`SELECT e.id, e.artifact_id, e.kind AS change, e.source, e.collector, e.provider, e.at,
                   x.file_name, x.kind, x.missing_at, x.abs_path
            FROM artifact_events e JOIN artifacts x ON x.id = e.artifact_id
            WHERE e.artifact_id IN (${scope})
            ORDER BY e.id DESC LIMIT ?`)
    .all(...params, limit)
    .map((row) => ({ ...row, ...describeArtifact(store, row.artifact_id) }));

  // 라이브러리가 숨긴 종류의 변경도 같은 기간에 몇 개 있었는지 알린다. 조용히 빼면 "방금 고친 파일이
  // 왜 안 보이나"가 된다(실제로 그랬다). 기간은 보여준 목록의 가장 오래된 줄부터다.
  let hidden = {};
  if (filters.view === 'library' && !filters.kind) {
    const since = changes.length === limit ? changes.at(-1).at : 0;
    ...
    hidden = Object.fromEntries(store.db
      .query(`SELECT x.kind, COUNT(*) AS n FROM artifact_events e JOIN artifacts x ON x.id = e.artifact_id
              WHERE e.at >= ? AND x.kind IN (${kinds}) GROUP BY x.kind`)
      .all(since, ...LIBRARY_HIDDEN_KINDS)
