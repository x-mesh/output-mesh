import { DEFAULT_SEARCH_LIMIT, MAX_TIMELINE_WEEKS, MIN_FTS_TOKEN_CHARS, OVERVIEW_ACTIVE_HOURS, OVERVIEW_RECENT_PERIOD, PERIOD_DAYS, nfc } from './paths.mjs';
...
const KIND_LABEL = { text: '문서', code: '코드', bundle: '폴더', markup: '웹 페이지', pdf: 'PDF', sheet: '스프레드시트', office: '오피스 문서', image: '이미지', other: '기타' };
...
export function facets(store, filters = {}) {
  return {
    ...
    kinds: facetGroup(store, filters, { kind: undefined, view: undefined }, "COALESCE(a.kind, '(미분류)')")
      .map((row) => ({ ...row, label: KIND_LABEL[row.value] ?? row.value })),
  ...
  return {
    ...
    activeConversations: store.db
      .query(`SELECT COUNT(DISTINCT o.collector || char(0) || ${CONVERSATION}) AS n
              FROM artifact_origins o WHERE o.occurred_at >= ?`)
