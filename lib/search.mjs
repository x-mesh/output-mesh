import { DEFAULT_CHANGE_LIMIT, DEFAULT_SEARCH_LIMIT, EXCERPT_CHARS, EXCERPT_LEAD_CHARS, MAX_TIMELINE_WEEKS, MIN_FTS_TOKEN_CHARS, OVERVIEW_ACTIVE_HOURS, OVERVIEW_RECENT_PERIOD, PERIOD_DAYS, SEARCH_BM25_WEIGHTS, nfc } from './paths.mjs';
import { KIND, plainText } from './extract.mjs';
import { describe, sessionTitleOf } from './describe.mjs';

const KIND_CODE = KIND.CODE;
const KIND_OTHER = KIND.OTHER;
/** 라이브러리가 기본으로 숨기는 종류. 사이드바가 '라이브러리 밖'으로 표시한다. */
export const LIBRARY_HIDDEN_KINDS = [KIND_CODE, KIND_OTHER, KIND.MEMO];
...
/**
 * 출처의 작업공간을 본 저장소로 접은 값. git worktree 와 저장소 하위 폴더에서 돈 세션이 한 저장소로
 * 모인다(실측: 작업공간 44곳 중 18곳이 worktree). 아직 찾지 않은 작업공간은 자기 자신이다.
 */
const rootOf = (alias) =>
  `COALESCE((SELECT w.root FROM workspace_roots w WHERE w.workspace = ${alias}.workspace), ${alias}.workspace)`;

/** 어느 출처 하나만 맞아도 걸린다. */
function originMatches(column) {
  const value = column === 'workspace' ? rootOf('o') : `o.${column}`;
  return `EXISTS (SELECT 1 FROM artifact_origins o WHERE o.artifact_id = a.id AND ${value} = ?)`;
}
...
function filterClauses(store, filters = {}) {
  ...
  if (filters.view === 'library' && !filters.kind) {
    // 라이브러리는 "문서·이미지·산출물"이다. 종류로만 거르면 세 번째가 빠진다 — 에이전트가
    // 산출물로 표시한 .json·.drawio 는 코드·기타로 분류돼 조용히 사라졌다. 표시는 긍정 주장이라
    // 종류를 이긴다.
    const byKind = LIBRARY_HIDDEN_KINDS.map((kind) => `a.kind IS NOT '${kind}'`).join(' AND ');
    where.push(`((${byKind}) OR EXISTS (
      SELECT 1 FROM artifact_origins o WHERE o.artifact_id = a.id AND o.is_deliverable = 1))`);
  }
  if (filters.kind) {
    where.push('a.kind = ?');
...
const likePattern = (token) => `%${token.replace(/[\\%_]/g, (c) => `\\${c}`)}%`;

/**
 * 검색 중의 순서. 검색어가 없으면 모두 0 이라 최근순이 그대로 남는다. 전문 검색은 bm25(작을수록
 * 잘 맞는다), 부분 일치는 bm25 가 없어서 어디에 맞았는지로 센다 — 이름, 작업 제목, 경로, 본문 순.
 *
 * bm25 는 MATERIALIZED CTE 로 한 번만 센다. 조인 안의 서브쿼리로 두면 SQLite 가 행마다 MATCH 를
 * 다시 돌려 802건짜리 질의가 3ms 에서 432ms 가 된다(측정).
 */
function relevance(plan) {
  if (plan.mode === 'fts') {
    return {
      cte: `hit AS MATERIALIZED (SELECT rowid AS id, bm25(artifact_fts, ${SEARCH_BM25_WEIGHTS.join(', ')}) AS score
        FROM artifact_fts WHERE artifact_fts MATCH ?),`,
      cteParams: [ftsExpression(plan.tokens)],
      join: 'LEFT JOIN hit ON hit.id = a.id',
      expr: 'hit.score',
      exprParams: [],
    };
  }
  if (plan.mode === 'like') {
    const perToken = "(CASE WHEN d.name LIKE ? ESCAPE '\\' THEN 0 WHEN d.meta LIKE ? ESCAPE '\\' THEN 1 WHEN d.path LIKE ? ESCAPE '\\' THEN 2 ELSE 3 END)";
    return {
      cte: '',
      cteParams: [],
      join: '',
      expr: plan.tokens.map(() => perToken).join(' + '),
      exprParams: plan.tokens.flatMap((token) => Array(3).fill(likePattern(token))),
    };
  }
  return { cte: '', cteParams: [], join: '', expr: '0', exprParams: [] };
}

/**
 * 본문에서 처음 맞은 검색어 주변. 이름이 겹치는 결과 14개 중 어느 것이 찾던 문서인지는 맞은
 * 대목이 말해 준다. snippet() 을 쓰지 않는다 — 한글 2글자가 가는 부분 일치 경로에는 없어서
 * 발췌가 두 벌이 된다.
 *
 * 안쪽 SELECT 의 LIMIT 1 은 행을 자르려는 게 아니다. 없으면 SQLite 가 서브쿼리를 펴서 lower(본문)
 * 을 참조하는 곳마다 다시 돌린다 — 802건에 낱말 둘이면 40ms 가 98ms 가 된다(측정).
 */
function excerptColumn(plan) {
  if (plan.tokens.length === 0) return { sql: 'NULL AS excerpt_json', params: [] };
  const positions = plan.tokens.map((_, i) => `instr(lb, lower(?)) AS p${i}`).join(', ');
  const first = plan.tokens.map((_, i) => `WHEN p${i} > 0 THEN p${i}`).join(' ');
  return {
    sql: `(SELECT CASE WHEN at > 0 THEN json_array(at, substr(d.body, max(1, at - ${EXCERPT_LEAD_CHARS}), ${EXCERPT_CHARS})) END
      FROM (SELECT CASE ${first} ELSE 0 END AS at
        FROM (SELECT ${positions} FROM (SELECT lower(d.body) AS lb LIMIT 1) LIMIT 1) LIMIT 1)) AS excerpt_json`,
    params: plan.tokens,
  };
}

function withExcerpt({ excerpt_json: json, ...row }) {
  if (!json) return { ...row, excerpt: null };
  const [at, text] = JSON.parse(json);
  const head = at > EXCERPT_LEAD_CHARS + 1 ? '…' : '';
  // 말줄임표는 잘라 온 원문 길이로 판단한다. 장식을 뗀 뒤 길이로 보면 안 잘린 발췌에도 붙는다.
  const tail = text.length === EXCERPT_CHARS ? '…' : '';
  const body = plainText(text).replace(/\s+/g, ' ').trim();
  return { ...row, excerpt: body ? `${head}${body}${tail}` : null };
}
...
export function search(store, input, filters = {}, limit = DEFAULT_SEARCH_LIMIT) {
  ...
  const rank = relevance(plan);
  const excerpt = excerptColumn(plan);

  // 보일 행을 먼저 자르고 그 행에만 출처를 장식한다. 대표를 뷰로 계산하면 SQLite 가 출처
  // 테이블 전체를 매 질의 구체화해 공급자 필터가 0.11ms 에서 2.40ms 로 느려진다(측정).
  const sql = `
    WITH ${rank.cte} page AS (
      SELECT a.id, ${rank.expr} AS score ${BASE_FROM} ${rank.join} ${whereSql}
      ORDER BY (a.state = 'final') DESC, score, a.mtime DESC LIMIT ?
    ),
    rep AS (
      SELECT * FROM (
    ...
    ORDER BY (a.state = 'final') DESC, page.score, a.mtime DESC`;
  return store.db
    .query(sql)
    .all(...rank.cteParams, ...rank.exprParams, ...params, limit, ...excerpt.params)
    .map((row) => withExcerpt(withDescription(withOriginLists(row))));
}
...
// 대표 출처 하나가 아니라 전부. 파일을 담은 저장소를 고르는 건 describe 다. top 은 그 체크아웃의
// 꼭대기(파일이 안에 있는지 보는 기준), root 는 이름을 빌려 올 본 저장소다.
// DISTINCT 를 안쪽에 둔다 — 집계에 걸면 json_object 가 JSON 이 아니라 문자열로 들어간다.
const WORKSPACES_EXPR = `(SELECT json_group_array(json_object('top', top, 'root', root)) FROM (
      SELECT DISTINCT COALESCE(w.top, o.workspace) AS top, COALESCE(w.root, o.workspace) AS root
      FROM artifact_origins o LEFT JOIN workspace_roots w ON w.workspace = o.workspace
      WHERE o.artifact_id = a.id AND o.workspace IS NOT NULL))`;
 ...
 */
function facetGroup(store, input, filters, drop, valueExpr, { join = '', extra = null, distinct = false } = {}) {
  const { where, params } = queryClauses(store, input, { ...filters, ...drop });
...
/** 검색어는 어느 차원도 아니라서 모든 그룹에 걸린다. 빼면 결과 14건 옆에서 사이드바가 전체 건수를 광고한다. */
export function facets(store, filters = {}, input = '') {
  const group = (...args) => facetGroup(store, input, filters, ...args);
  return {
    ...
    kinds: group({ kind: undefined, view: undefined }, "COALESCE(a.kind, 'unclassified')"),
    collectors: group({ collector: undefined }, 'o.collector', ORIGIN_FACET),
    providers: group({ provider: undefined }, 'o.provider', { ...ORIGIN_FACET, extra: 'o.provider IS NOT NULL' }),
    workspaces: group({ workspace: undefined }, rootOf('o'), { ...ORIGIN_FACET, extra: 'o.workspace IS NOT NULL' }).slice(0, 12),
    states: group({ state: undefined }, 'a.state'),
    exts: group({ ext: undefined }, 'a.ext', { extra: "a.ext <> ''" }),
    tags: group({ tag: undefined }, 't.name', { join: TAG_JOIN }),
  };
}
...
export function timeline(store, input, filters = {}, period = 'all', now = new Date()) {
  ...
  for (const column of ORIGIN_SCOPED_FILTERS) {
    ...
    where.push(`${column === 'workspace' ? rootOf('t') : `t.${column}`} = ?`);
...
export function activity(store, { limit = ACTIVITY_SESSION_LIMIT, collector, workspace, since } = {}) {
  ...
  return sessions.map((session) => ({
    ...session,
    session_title: sessionTitleOf(session.session_title),
