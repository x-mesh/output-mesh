...
- **변경 기록은 `[Unreleased]` 에 미리 적는다.** `git-kit ship` 은 그 칸이 **비어 있을 때만** 커밋 제목으로 초안을 만든다. 이 저장소의 커밋 제목은 한국어라 초안이 그대로 남으면 나머지 영어 항목과 어긋난다(실제로 0.4.1~0.6.0 이 그렇게 섞였다). 일이 끝날 때마다 `**scope:** 무엇이 달라졌나` 한 줄을 영어로 적어 두면 릴리스가 그걸 그대로 올린다.
