...
<p align="center">
  <img src="./assets/demo.gif" alt="output-mesh: 라이브러리 탐색, 산출물과 그것을 만든 에이전트·세션 확인, 작업으로 검색, 새 파일이 실시간으로 등장" width="900" />
</p>

<p align="center"><sub>실제 로그가 아닌 격리된 예시 데이터로 녹화했다.</sub></p>
...
검색어를 넣으면 트리 대신 관련도순 목록이 된다. 결과마다 위치와 본문에서 맞은 대목이 붙고, 열면 미리보기가 맞은 곳을 칠하고 첫 일치로 스크롤한다.

**개요(오른쪽, 아무것도 고르지 않았을 때).** "방금 일어난 일"이 먼저다. 생기고 바뀌고 옮겨지고 사라진 파일을 어느 에이전트인지와 함께 보여주고, 스크롤하면 30일 전까지 거슬러 간다. 옆에는 최종본이 있다. 에이전트가 쓴 지 4시간이 넘어서야 알게 된 변경은 여기에 올리지 않는다. 그 아래는 그래프다. 기간에 따라 시간, 날, 주 단위로 본 에이전트 활동과 종류 · 에이전트 · 작업공간 분포이고, 막대를 누르면 그대로 필터가 된다.

**상세(오른쪽, 파일을 골랐을 때).** 미리보기가 화면을 차지한다. 마크다운은 렌더해서 보여주고 프런트매터는 표로 정리한다. 정보 패널에는 이 파일을 건드린 모든 세션과 태그, 메모, 최종본 표시가 있다.

**활동.** 에이전트 세션과 각 세션이 쓴 파일을 실시간 타임라인으로 본다. 고른 파일이 없으면 타임라인이 화면 전체를 쓰고, 파일을 고르면 타임라인은 왼쪽 칸으로 물러나고 오른쪽에 파일이 열린다.

수집 중에 오류가 나면 상단에 "수집 오류"가 한 시간 동안 보인다. 누르면 최근 오류 목록이 나온다.
...
- 에이전트가 셸로 쓴 파일은 로그에 경로가 남지 않는다. 문서는 저장소 감시로 찾지만 세션 출처는 붙지 않고, 그렇게 쓴 소스 코드는 모으지 않는다.
- 스프레드시트 미리보기는 값만 보여준다(시트마다 처음 200행 · 30열). 서식, 병합, 차트는 없다.
